...
def sample_dataset_random(x: np.ndarray[float],
                          n_samples: int, random_seed: int = None) -> list[int]:
    rng = np.random
    if random_seed:
...
def sample_dataset_jls_kmeans(x: np.ndarray[float],
    ...
    return sorted([int(rng.choice(cluster_indices)) for cluster_indices in indices_per_cluster.values()])
